...
- 支持的扩展名：orchestrator.knowledge_pipeline.DEFAULT_SUPPORTED_EXTS；
- collection：默认取 db.qdrant 的 config.default_collection。

用法：
  python -m scripts.upload_files --path ./docs                      # 递归上传整个目录
  python -m scripts.upload_files --path ./a.pdf                     # 单个文件
  python -m scripts.upload_files --path ./docs --collection knowledgebase
  python -m scripts.upload_files --path ./docs --dry-run            # 只看将上传什么，不实际传
"""
...
from orchestrator.knowledge_pipeline import DEFAULT_SUPPORTED_EXTS
...
def main() -> None:
    ...
    print("下一步（入库）：")
    print("  python -m orchestrator.knowledge_pipeline")
    print(f"  python -m scripts.chunk_minio_to_qdrant --collection {collection}")
