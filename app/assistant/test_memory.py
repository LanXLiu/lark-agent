...
import app.assistant.memory as memory
...
def test_empty_session_has_empty_views():
    mem = _mem()
    assert mem.get_for_rewrite("k") == []
    assert mem.get_for_answer("k").is_empty()


def test_rewrite_view_returns_recent_rewrite_turns():
...
def test_answer_view_returns_full_turns_within_budget():
...
def test_session_keys_are_isolated():
...
def test_summary_is_created_after_token_threshold():
...
def test_summary_keeps_old_turns_in_summary_instead_of_dropping_them():
...
def test_without_summary_old_turns_are_trimmed_by_token_budget():
...
def test_single_oversized_turn_still_keeps_latest_turn():
...
def test_concurrent_append_during_summary_is_not_deleted():
...
def test_summary_failure_keeps_recent_turns():
    mem = _mem(summary_enabled=True, summary_trigger_tokens=100,
...
def test_repeated_summary_failure_does_not_grow_without_bound():
    mem = _mem(summary_enabled=True, summary_trigger_tokens=100,
               summarizer=_FailingSummarizer())
    for _ in range(200):
        mem.append_turn("k", "x" * 30, "")
...
def test_expired_ttl_starts_new_topic():
...
def test_exceeding_max_sessions_evicts_least_recently_used_session():
...
def test_clear_removes_selected_session():
    mem = _mem(summary_trigger_tokens=10_000)
...
def test_sqlite_persistence_restores_session_after_rebuild(tmp_path):
...
def test_clear_also_removes_persisted_session(tmp_path):
    from app.assistant.memory_store import SQLiteMemoryStore
    db = str(tmp_path / "mem.db")
...
def test_expired_persisted_session_is_not_restored_after_restart(tmp_path):
    import time as _t
    from app.assistant.memory_store import SQLiteMemoryStore
    db = str(tmp_path / "mem.db")
    store = SQLiteMemoryStore(db)
...
def test_flush_runs_before_summary_and_merges_facts():
