...
def test_answers_after_single_tool_call():
    llm = _FakeLLM([
...
def test_runs_multiple_tool_call_rounds():
    llm = _FakeLLM([
...
def test_disables_tools_on_final_round_limit():
...
def test_no_retrieval_hits_marks_no_answer():
...
def test_high_score_hit_overrides_llm_no_answer():
...
def test_chitchat_answers_without_tool_call():
...
def test_invalid_tool_arguments_do_not_crash_loop():
    llm = _FakeLLM([
...
def test_falls_back_to_web_search_when_knowledge_is_insufficient(monkeypatch):
    # 知识库召回为空 → LLM 说没找到 → finalize 降级调 web_search → 用联网结果作答、带联网来源。
    import app.assistant.agent.tools.registry as reg
    from app.assistant.agent.tools.base import ToolResult
...
def test_web_search_disabled_keeps_no_answer_when_knowledge_is_insufficient(monkeypatch):
    class _EmptyRecaller:
        def search(self, request):
            return RecallResult(query=request.query, collection=request.collection,
                                hits=[], total=0, latency_ms=1.0,
