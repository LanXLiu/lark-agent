...
from knowledge.ingestion.chunker.base import ChunkResult
...
def deduplicate_document_chunks(
    ...
    *,
    ...
    try:
        from knowledge.utils.lsh_deduplication import deduplicate_chunks_with_lsh
...
def _should_drop_short_or_heading_chunk(
    ...
    # 标题匹配只用真实 title 或 heading-only 块的标题；短正文块不能拿全文充当
    # 标题（否则精确重复会误删「第一个」而非保留它，交由下方的包含规则处理）。
    candidate_title = title or heading_title or ""
