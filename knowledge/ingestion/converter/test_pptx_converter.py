...
用法（在项目根目录）::
    ...
    python -m knowledge.ingestion.converter.test_pptx_converter /path/to/foo.pptx
    python -m knowledge.ingestion.converter.test_pptx_converter foo.pptx --visual
    python -m knowledge.ingestion.converter.test_pptx_converter foo.pptx --visual --dpi=240
...
def main() -> None:
    ...
    if not positional:
        ...
        if default_ppt.is_file():
            ...
            print("用法: python -m knowledge.ingestion.converter.test_pptx_converter <本地 ppt 路径> [--visual] [--dpi=200]")
            sys.exit(1)
    else:
        p = Path(positional[0])
