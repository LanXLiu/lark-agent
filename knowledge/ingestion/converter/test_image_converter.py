...
用法（在项目根目录）::
    ...
    python -m knowledge.ingestion.converter.test_image_converter /path/to/image.png
...
def main() -> None:
    ...
    if not argv:
        ...
        if default_png.is_file():
            ...
            print("用法: python -m knowledge.ingestion.converter.test_image_converter <本地图片路径>")
            sys.exit(1)
    else:
        p = Path(argv[0])
