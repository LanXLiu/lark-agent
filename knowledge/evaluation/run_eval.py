...
流程：读本地测试集 → 逐条调用 app.assistant.qa_service 拿到「答案 + 检索片段 + 来源」
...
用法：
  ...
  python -m knowledge.evaluation.run_eval
...
from knowledge.evaluation.datamodel import EvalSample, load_testset
...
def _build_qa_service(collection: str):
    ...
    from app.assistant import AgentService
...
def run(testset_path: str, collection: str, out_dir: str) -> None:
    ...
    from knowledge.evaluation.ragas_setup import build_judge_embeddings, build_judge_llm
