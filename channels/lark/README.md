# channels/lark — 飞书接入通道

飞书机器人通道：接收群消息、触发 RAG 问答、回复带来源和反馈按钮的答案，以及下载群文档入库。通过 lark-oapi 长连接（websocket）与飞书通信，无需公网入站。

## 文件

| 文件 | 职责 |
|---|---|
| `bot.py` | **入口**。长连接客户端；收消息 → 去重 → 快速 ack → 入**有界队列**（削峰）→ 固定 worker 线程用**单例** Agent 服务（`service.agent`）跑问答；卡片按钮回调（反馈）。队列满时优雅拒绝。`python -m channels.lark.bot` 启动。 |
| `lark_api.py` | 飞书 OpenAPI 封装：tenant_access_token、发消息、贴表情、取云文档原生 markdown、群名/文档名查询。 |
| `download.py` | 下载群文档/附件到 MinIO。`python -m channels.lark.download` 可单独运行。 |
...
问答不在本模块实现——`bot.py` 把问题交给 `service.agent`（Agent 工具调用），本模块只负责「飞书这一侧」的收发。
