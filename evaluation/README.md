...
评估跑的是线上那套 Agent 工具调用问答（`service.agent`），所以评的就是机器人实际的召回 + 生成（评测时关闭降级联网，只评知识库检索质量）。

## 数据放哪里

评估需要一份真实问答测试集。为便于版本管理，仓库把「示例」和「你自己的数据」分开：

- 仓库自带 `evaluation/testset.example.jsonl`：几条示范格式的样例，照着它写；
...
```bash
mkdir -p eval_data
cp evaluation/testset.example.jsonl eval_data/testset.jsonl
...
python -m evaluation.run_eval             # 默认读 eval_data/testset.jsonl
# 或指定：
