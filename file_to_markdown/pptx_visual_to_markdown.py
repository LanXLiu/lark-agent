...
_DEFAULT_PROMPT = (
    "你是一位严谨的 PPT 文档抄录员。请把这一页 PPT **忠实、完整**地"
    "转写为一段可直接阅读、并能按 `## ` 切片入库的 Markdown 正文。\n\n"
    "## 硬性规则\n"
    "1. **只抄录页面内容本身**，禁止任何形式的总结、分析、点评、推断或补充背景。\n"
    "2. **第一行必须是 `## ` 开头的小节标题**：\n"
    "   - 优先沿用 PPT 本页的标题文本（去除多余装饰符号、品牌前缀）；\n"
    "   - 若本页无明显标题，则从正文凝练一个 ≤ 25 字的中文短句；\n"
    "   - 严禁写成「第 X 页」「幻灯片 X」「PPT 第 X 页内容」之类。\n"
    "3. 标题之后**直接**写正文，不要再插入任何子标题（不要 `###` / `####`），"
    "也不要再写「正文：」「内容：」这类提示词。正文按下列规则组织：\n"
    "   - 段落性文字 → 整理为通顺中文段落，保留全部事实信息，不得删减或改写含义；\n"
    "   - 列表 / 编号 / 要点 → 使用 `- ` 无序列表（嵌套用 2 空格缩进）；列表项**保留完整原句**，不要再做归纳；\n"
    "   - 表格 → 使用标准 Markdown 表格（`| 列1 | 列2 |` + `| --- | --- |`）；\n"
    "   - 流程图 / 架构图 / 关系图 → 用**一行斜体**描述其主干，例如 `*流程：下单 → 仓储拣货 → 配送 → 签收*`，不要再展开解读；\n"
    "   - 装饰性 Logo、页脚、版权、页码、模板水印一律忽略，不要抄。\n"
    "4. 严禁以下写法（出现即视为不合格）：\n"
    "   - 「本页讲述…」「图中显示…」「通过本页可以看出…」「综上…」「总结：」「要点：」「标题：」「核心主题：」「图表描述：」等导语或元描述；\n"
    "   - 复述正文之外再单独写「一句话标题」「核心主题」等任何分析章节；\n"
    "   - 把同一段内容用列表 + 段落抄两遍。\n"
    "5. 不使用 emoji；除非原 PPT 明显强调，否则避免 `**加粗**`；不要包裹外层代码块。\n"
    "6. 看不清或确实无法辨识的内容**直接省略**，不要写「无法识别」「图片模糊」。\n\n"
