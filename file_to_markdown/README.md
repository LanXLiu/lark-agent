# file_to_markdown — 转 Markdown 核心

把各类文件内容真正渲染成 markdown 的实现层。含 VLM 识图、表格渲染、后处理等。被 `converter` 调用。
...
VLM 相关配置（url / api_key / model）在 `conf/config_local.yaml` 的 `MODELS.VLM` 段；未配置时视觉转换自动回退到纯文本方案。
