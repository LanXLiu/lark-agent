...
从 ``conf/config_{ENV}.yaml`` 读取环境相关配置（ENV 缺省为 local），
将 YAML 顶层键值动态挂到 ``Settings`` 类上，供 ``from conf.settings import settings`` 使用。
"""
...
from conf.env_interpolate import interpolate_env
...
def config_yaml_path() -> str:
    ...
    Returns:
        ``conf/config_{ENV}.yaml`` 的完整路径字符串。
    """
    env = load_env()
